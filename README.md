...
Для установки сборки потребуется `bower` или `npm`:

    > npm install yatra --save-dev
    > bower install yatra --save
...
- [х] автоматизивароть сборку и зарегистрировать в bower
- [х] прогон тестов используя node.js (зарегистрировать в npm)
...
Если исключение будет выброшено, то утвержение будет считаться верным. Иначе будет считаться ошибкой. При возникновении исключений внутри функции обернутой `assert.exception` выполнение остального кода теста не прерывается.
